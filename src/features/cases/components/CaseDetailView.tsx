...
import { CalendarDays, Clock3, Gavel, MessageSquare, Plus, Save, Wallet } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { auth } from "@/lib/firebase/client";
import { useCaseAutosave } from "@/features/cases/hooks/useCaseAutosave";
import { CASE_STAGE_OPTIONS, CASE_STATUS_OPTIONS, type CaseRecord, type CaseStage, type CaseStatus, type TimelineEventRecord, type TimelineType } from "@/features/cases/types";
  ...
  async function createTimelineEvent() {
    ...
    try {
      await postTimeline({
        ...
        description: "Hito creado desde la ficha de la causa.",
        type: "observacion",
      });
      setNewTimelineTitle("");
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[32px] bg-white p-6 shadow-sm dark:bg-slate-900">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          <Button variant="outline" className="rounded-2xl" onClick={() => void createTimelineEvent()} disabled={busyAction === "timeline"}><Plus className="mr-2 h-4 w-4" />Agregar hito</Button>
          ...
          <Button variant="outline" className="rounded-2xl" onClick={() => void postTimeline({ title: "Aviso al cliente", description: "Se registra aviso o comunicacion al cliente.", type: "observacion", visibleToClient: true })}><MessageSquare className="mr-2 h-4 w-4" />Avisar cliente</Button>
        </div>
      </div>
        ...
        <div className="mt-6 space-y-4">
          {events.map((event) => (
            <div key={event.id} className="rounded-3xl border border-neutral-200 bg-neutral-50 p-4 dark:border-white/10 dark:bg-slate-950">
              ...
              {event.linkedAgendaEventId && <p className="mt-2 text-xs font-medium text-emerald-600">Agenda vinculada: {event.linkedAgendaEventId.slice(0, 8)}</p>}
            </div>
          ))}
          {events.length === 0 && <div className="rounded-3xl border border-dashed border-neutral-200 p-8 text-center text-neutral-400">Sin hitos registrados.</div>}
        </div>
      </div>
    </div>
  );
